import { View, Image, TextInput, Text, TouchableOpacity, ScrollView, HorizontalScrollView, Modal, RefreshControlComponent } from "react-native";
...
export default function repair({ navigation }) {
  ...
  let isUser = firebase.auth().currentUser.uid;
  ...
  const goRePair = async () => {
    ...
    console.log("numbernisit", numbernisit);
    ...
    try {
      const userRef = doc(firebase.firestore(), "users", isUser);//ตรวจสอบข้อมูลของผู้ใช้
      const userDoc = await getDoc(userRef);
      let my_report = [];
      console.log("userDoc.data().my_report: ",userDoc.data().my_report);
      if (userDoc.exists() && userDoc.data().my_report) {
        my_report = userDoc.data().my_report;
      }
      my_report.push({
        kana: kana,
        numbernisit: numbernisit,
      ...
      const AdminRef = doc(firebase.firestore(), "users", 'MjUWBCwkmJhgGcNPmg4LUNO6jUT2');//ตรวจสอบข้อมูลของแอดมิน
      const AdminDoc = await getDoc(AdminRef);
      let report = [];
      if (AdminDoc.exists() && AdminDoc.data().report) {
        report = AdminDoc.data().report;
      }
      report.push({
        kana: kana,
        numbernisit: numbernisit,
